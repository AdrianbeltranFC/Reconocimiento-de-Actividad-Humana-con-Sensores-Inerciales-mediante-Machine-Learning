...
"""
Interfaz con corrección para bloqueo por exceso de señales BLE:
 - AsyncBLEClient ahora almacena muestras en cola thread-safe.
 - ModeBWidget procesa por lotes (batch) desde esa cola en el hilo principal.
 - Throttling de UI (plot_timer y table_timer).
 - Tres gráficas: Acc, Gyro, Ang; tabla formateada; guardado CSV.
"""
...
from PyQt5.QtWidgets import (
    ...
    QStackedWidget, QFileDialog, QMessageBox, QSizePolicy, QHeaderView
)
...
# Async BLE client with queue (thread-safe) to avoid emitting per-sample signals
class AsyncBLEClient(QObject):
    status = pyqtSignal(str)  # still use for status messages
    ...
    # Start event loop in background thread (same as before)
    ...
    # Bring newest samples from internal queue (called by main thread)
    def fetch_samples(self, max_n: int = 100) -> List[Dict[str, Any]]:
        out = []
        with self._queue_lock:
            for _ in range(min(max_n, len(self._sample_queue))):
    ...
    async def _connect_and_listen(self, address: str, notify_uuid: str):
        try:
            ...
            def _notify_cb(sender, data: bytearray):
                # Very lightweight: push raw bytes / parsed dict into queue with lock.
                try:
                    # Try using repo parser if present
                    parsed = None
                    if self._repo_parser is not None:
                        try:
                            parsed = self._repo_parser(data)
                        except Exception:
                            parsed = None
                    ...
                    if parsed is not None and isinstance(parsed, dict):
                        ...
                        try:
                            ...
                            if len(parts) >= 6:
                                ...
                                # fallback: store raw bytes for main thread parsing
                                item = {"raw_bytes": bytes(data), "time": time.time()}
                        except Exception:
                            item = {"raw_bytes": bytes(data), "time": time.time()}
                    with self._queue_lock:
                        self._sample_queue.append(item)
                        # keep queue to reasonable size to avoid growing unbounded
                        if len(self._sample_queue) > 20000:
                            # drop oldest
                            self._sample_queue.popleft()
                except Exception:
                    # swallow; don't block BLE thread
                    pass
...
# Mode A (unchanged from previous stable version)
...
# Mode B with client queue + batched processing
class ModeBWidget(QWidget):
    def __init__(self, go_home_callback):
        ...
        self.ang_plot.addLegend(offset=(10, 10))

        self.acc_curves = {
            'x': self.acc_plot.plot(pen=pg.mkPen('#0066CC', width=1.8), name='Acc X (g)'),
            'y': self.acc_plot.plot(pen=pg.mkPen('#00CC99', width=1.8), name='Acc Y (g)'),
            'z': self.acc_plot.plot(pen=pg.mkPen('#FF9933', width=1.8), name='Acc Z (g)')
        }
        self.gyro_curves = {
            'x': self.gyro_plot.plot(pen=pg.mkPen('#0066CC', width=1.8), name='Gyro X (°/s)'),
            'y': self.gyro_plot.plot(pen=pg.mkPen('#00CC99', width=1.8), name='Gyro Y (°/s)'),
            'z': self.gyro_plot.plot(pen=pg.mkPen('#FF9933', width=1.8), name='Gyro Z (°/s)')
        }
        self.ang_curves = {
            'x': self.ang_plot.plot(pen=pg.mkPen('#0066CC', width=1.8), name='Angle X (°)'),
            'y': self.ang_plot.plot(pen=pg.mkPen('#00CC99', width=1.8), name='Angle Y (°)'),
            'z': self.ang_plot.plot(pen=pg.mkPen('#FF9933', width=1.8), name='Angle Z (°)')
        }
        ...
        self.max_table_rows = 200
        header = self.table.horizontalHeader()
        header.setSectionResizeMode(0, QHeaderView.Stretch)
        for c in range(1, len(self.ui_columns)):
            header.setSectionResizeMode(c, QHeaderView.ResizeToContents)

        # status
        self.status_label = QLabel("Muestras: 0 — Último: —")
        ...
        # state
        self.ble_client: Optional[AsyncBLEClient] = None
        ...
        self.plot_timer.timeout.connect(self._process_batch_and_plot)

        self.table_timer = QTimer()
        self.table_timer.setInterval(500)
        self.table_timer.timeout.connect(self._table_update)
        ...
        self._max_buffer_rows = 8000
        self._norm_state = {"last_time": None}

        # batch size per plot tick - smaller reduces UI pressure
        self._batch_size = 20
    ...
    def _process_batch_and_plot(self):
        """
        Called by plot_timer in GUI thread.
        - Fetches up to batch_size samples from ble_client queue
        - Normalizes, appends to normalized_buffer and plotting arrays (fast operations)
        - Updates plots (batched)
        """
        try:
            if self.simulate:
                # generate simulated sample infrequently
                t = time.time()
                samp = {
                ...
                if not self.ble_client:
                    return
                samples = self.ble_client.fetch_samples(self._batch_size)
            if not samples:
                # still update plots from arrays if any
                self._update_plots_from_arrays()
                return

            # process each sample quickly
                ...
                try:
                    if "raw_bytes" in sample:
                        ...
                        parsed = {
                            ...
                            "time": sample.get("time", time.time())
                        }
                        norm = normalize_parsed(parsed, self._norm_state, device_name=self.mac_input.text().strip())
                    else:
                        norm = normalize_parsed(sample, self._norm_state, device_name=self.mac_input.text().strip())

                    # append normalized row
                    self.normalized_buffer.append(norm)
                    if len(self.normalized_buffer) > self._max_buffer_rows:
                        del self.normalized_buffer[0:len(self.normalized_buffer) - self._max_buffer_rows]

                    # append numeric arrays
                    tnum = norm.get("timestamp", time.time())
                    ax = try_float(norm.get("Acceleration X(g)")) if norm.get("Acceleration X(g)") != "" else np.nan
                    ay = try_float(norm.get("Acceleration Y(g)")) if norm.get("Acceleration Y(g)") != "" else np.nan
                    az = try_float(norm.get("Acceleration Z(g)")) if norm.get("Acceleration Z(g)") != "" else np.nan
                    gx = try_float(norm.get("Angular velocity X(°/s)")) if norm.get("Angular velocity X(°/s)") != "" else np.nan
                    gy = try_float(norm.get("Angular velocity Y(°/s)")) if norm.get("Angular velocity Y(°/s)") != "" else np.nan
                    gz = try_float(norm.get("Angular velocity Z(°/s)")) if norm.get("Angular velocity Z(°/s)") != "" else np.nan
                    angx = try_float(norm.get("Angle X(°)")) if norm.get("Angle X(°)") != "" else np.nan
                    angy = try_float(norm.get("Angle Y(°)")) if norm.get("Angle Y(°)") != "" else np.nan
                    angz = try_float(norm.get("Angle Z(°)") ) if norm.get("Angle Z(°)") != "" else np.nan

                    self.time_buff.append(tnum)
                    ...
                    # trim plot arrays
                    if len(self.time_buff) > self._max_plot_points:
                        for lst in (self.time_buff, self.acc_x, self.acc_y, self.acc_z,
                                    self.gyro_x, self.gyro_y, self.gyro_z,
                                    self.ang_x, self.ang_y, self.ang_z):
                            del lst[0:len(lst) - self._max_plot_points + 1]
                except Exception:
                    # continue processing remaining samples even if one fails
                    continue

            # After batch appended, update plots once
            self._update_plots_from_arrays()
        except Exception as e:
            print("Batch process error:", e)

    def _update_plots_from_arrays(self):
        try:
    ...
    def _table_update(self):
        try:
            n = len(self.normalized_buffer)
            if n == 0:
                return
            last = self.normalized_buffer[-1]
            last_str = last.get("ime", format_time(time.time()))
            self.status_label.setText(f"Muestras: {n} — Último: {last_str}")

            display_rows = self.normalized_buffer[-self.max_table_rows:]
            if self.table.rowCount() != len(display_rows):
                self.table.setRowCount(len(display_rows))
            for i, row in enumerate(display_rows):
                for j, col in enumerate(self.ui_columns):
                    val = row.get(col, "")
                    item = QTableWidgetItem(str(val))
                    if col != "ime" and col != "Device name" and col != "Chip Time()":
                        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
                        vnum = try_float(val) if val != "" else None
                        if vnum is not None:
                            item.setText(f"{vnum:.6g}")
                    else:
